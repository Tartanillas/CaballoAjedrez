...
function winnerPosition() {
	const columnas = ['A', 'B', 'C', 'E', 'F', 'G', 'H']
	const filas = ['1', '2', '3', '4', '5', '6', '7', '8']
	const columnaAleatoria = columnas[parseInt(Math.random() * columnas.length)]
	const filaAleatoria = filas[parseInt(Math.random() * filas.length)]
	const casillaResultante = columnaAleatoria + filaAleatoria
	const casilla = document.getElementById(casillaResultante).classList.add('final')
	return casilla
}